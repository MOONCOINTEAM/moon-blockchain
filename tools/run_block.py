...
"""
run_block: Convert an encoded FullBlock from the Moon blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../moon/types/full_block.py) in json format
...
$ca_root is the directory containing your current Moon config files
$hash is the header_hash of the [BlockRecord](../moon/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../moon/types/condition_opcodes.py)
...
from moon.full_node.generator import create_generator_args
...
from moon.types.condition_with_args import ConditionWithArgs
...
from moon.wallet.cat_wallet.cat_utils import match_cat_puzzle
