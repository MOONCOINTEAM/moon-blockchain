...
from moon.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from moon.plotters.moonpos import get_moonpos_install_info, plot_moon
from moon.plotters.madmax import get_madmax_install_info, plot_madmax
from moon.plotters.install_plotter import install_plotter
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        u_default = 0 if name == "moonpos" else 256
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in MOON_ROOT.
    moon_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within MOON_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, moon_plotter, "moonpos", "Moonpos Plotter")
    ...
    if args.plotter == "moonpos":
        plot_moon(args, moon_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, moon_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, moon_root_path, root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    moonpos: Optional[Dict[str, Any]] = get_moonpos_install_info()
