...
from moon.data_layer.data_layer_errors import OfferIntegrityError
from moon.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from moon.types.blockchain_format.program import Program, SerializedProgram
...
from moon.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from moon.wallet.lineage_proof import LineageProof
from moon.wallet.outer_puzzles import AssetType
from moon.wallet.puzzle_drivers import PuzzleInfo, Solver
from moon.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from moon.wallet.sign_coin_spends import sign_coin_spends
from moon.wallet.trading.offer import NotarizedPayment, Offer
from moon.wallet.transaction_record import TransactionRecord
from moon.wallet.util.compute_memos import compute_memos
from moon.wallet.util.merkle_utils import _simplify_merkle_proof
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSMoonConnection) -> CoinState:
    ...
    async def create_tandem_moc_tx(
        self,
        fee: uint64,
        announcement_to_assert: Announcement,
        coin_announcement: bool = True,
    ) -> TransactionRecord:
        moon_tx = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
        ...
        assert moon_tx.spend_bundle is not None
        return moon_tx
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            moon_tx = await self.create_tandem_moc_tx(
                fee, Announcement(current_coin.name(), b"$"), coin_announcement=True
            )
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, moon_tx.spend_bundle])
            dl_tx = dataclasses.replace(dl_tx, spend_bundle=aggregate_bundle)
            moon_tx = dataclasses.replace(moon_tx, spend_bundle=None)
            txs: List[TransactionRecord] = [dl_tx, moon_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSMoonConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            moon_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, moon_tx.spend_bundle])
                ),
                dataclasses.replace(moon_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSMoonConnection) -> None:
