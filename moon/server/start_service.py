...
from moon.daemon.server import service_launch_lock_path
from moon.util.lock import Lockfile, LockfileError
from moon.server.ssl_context import moon_ssl_ca_paths, private_ssl_ca_paths
...
from moon.cmds.init_funcs import moon_full_version_str
from moon.rpc.rpc_server import start_rpc_server
from moon.server.outbound_message import NodeType
from moon.server.server import MoonServer
from moon.server.upnp import UPnP
...
class Service:
    def __init__(
        ...
        *,
        ...
        self._log.info(f"moon-blockchain version: {moon_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        moon_ca_crt, moon_ca_key = moon_ssl_ca_paths(root_path, self.config)
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"moon_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for MoonServer to be closed")
