...
from moon.full_node.full_node_api import FullNodeAPI
from moon.protocols.full_node_protocol import RespondBlock
from moon.simulator.block_tools import BlockTools
from moon.simulator.simulator_protocol import FarmNewBlockProtocol, GetAllCoinsProtocol, ReorgProtocol
...
from moon.util.api_decorators import api_request
from moon.util.config import lock_and_load_config, save_config
from moon.util.ints import uint8, uint32, uint64, uint128
from moon.wallet.transaction_record import TransactionRecord
from moon.wallet.util.wallet_types import AmountWithPuzzlehash
...
class FullNodeSimulator(FullNodeAPI):
    ...
    async def process_blocks(self, count: int, farm_to: bytes32 = bytes32([0] * 32)) -> int:
        ...
        Consider `.farm_blocks()` or `.farm_rewards()` if the goal is to receive MOC at
        an address.
