...
from moon.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
...
from moon.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from moon.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from moon.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from moon.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from moon.wallet.payment import Payment
from moon.wallet.puzzle_drivers import PuzzleInfo, Solver
from moon.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from moon.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
from moon.wallet.transaction_record import TransactionRecord
from moon.wallet.util.compute_memos import compute_memos
from moon.wallet.util.debug_spend_bundle import disassemble
...
class NFTWallet:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSMoonConnection) -> None:
    ...
    async def create_tandem_moc_tx(
        ...
        moon_coins = await self.standard_wallet.select_coins(fee)
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, moon_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if moon_tx is not None and moon_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, moon_tx.spend_bundle])
        ...
        if moon_tx is not None:
            tx_list.append(moon_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            moon_tx = await self.create_tandem_moc_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, moon_tx
    ...
    async def make_nft1_offer(
        ...
        if offering_nft:  # if we are offering an NFT.
            ...
            if requested_asset_id is None:  # If we are just asking for moc.
            ...
            if offered_asset_id is None:  # moc offer
