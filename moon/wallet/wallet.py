...
from moon.full_node.bundle_tools import simple_solution_generator
...
from moon.wallet.puzzles.puzzle_utils import (
...
from moon.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_moon(self, moon_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if moon_amount < 0:
            utxos = await self.select_coins(uint64(abs(moon_amount)), exclude)
        else:
        ...
        moon_amount = spend_value + moon_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(moon_amount), "memos": []}
                ]
