...
from moon.consensus.blockchain import ReceiveBlockResult
from moon.consensus.constants import ConsensusConstants
from moon.daemon.keychain_proxy import (
    KeychainProxy,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from moon.protocols import wallet_protocol
from moon.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from moon.protocols.protocol_message_types import ProtocolMessageTypes
from moon.protocols.wallet_protocol import (
    CoinState,
    RespondBlockHeader,
    RespondToCoinUpdates,
    RespondToPhUpdates,
)
from moon.server.node_discovery import WalletPeers
...
from moon.types.peer_info import PeerInfo
from moon.types.weight_proof import WeightProof
from moon.util.byte_types import hexstr_to_bytes
from moon.util.chunks import chunks
from moon.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from moon.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
from moon.util.ints import uint32, uint64
...
from moon.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from moon.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from moon.wallet.util.wallet_sync_utils import (
...
from moon.wallet.wallet_state_manager import WalletStateManager
from moon.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    _server: Optional[MoonServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'moon keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSMoonConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns.mooncoin.top"]),
    ...
    def on_disconnect(self, peer: WSMoonConnection):
    ...
    async def on_connect(self, peer: WSMoonConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSMoonConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSMoonConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSMoonConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSMoonConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSMoonConnection]:
        """
        ...
        synced_and_trusted: List[WSMoonConnection] = []
        synced: List[WSMoonConnection] = []
        trusted: List[WSMoonConnection] = []
        neither: List[WSMoonConnection] = []
        all_nodes: List[WSMoonConnection] = self.server.get_full_node_connections().copy()
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSMoonConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSMoonConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSMoonConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSMoonConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSMoonConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSMoonConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSMoonConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSMoonConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
