...
from moon.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from moon.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            moon_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        moon_tx = dataclasses.replace(moon_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(moon_tx)
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSMoonConnection):
    ...
    async def create_tandem_moc_tx(
        ...
        if fee > amount_to_claim:
            moon_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(moon_coins)[0].name()
            ...
            message = None
            for spend in moon_tx.spend_bundle.coin_spends:
            ...
            moon_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in moon_coins])
            moon_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some MOC as part of this
        regular_moon_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_moon_to_claim = payment_amount

        need_moon_transaction = (fee > 0 or regular_moon_to_claim > 0) and (fee - regular_moon_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_moon_transaction:
                    if fee > regular_moon_to_claim:
                        moon_tx, _ = await self.create_tandem_moc_tx(
                            fee,
                            uint64(regular_moon_to_claim),
                    ...
                    elif regular_moon_to_claim > fee:
                        moon_tx, _ = await self.create_tandem_moc_tx(
                            fee, uint64(regular_moon_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        moon_spend_bundle = SpendBundle([], G2Element())
        if moon_tx is not None and moon_tx.spend_bundle is not None:
            moon_spend_bundle = moon_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if moon_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=moon_tx.confirmed_at_height,
                    created_at_time=moon_tx.created_at_time,
                    to_puzzle_hash=moon_tx.to_puzzle_hash,
                    amount=moon_tx.amount,
                    fee_amount=moon_tx.fee_amount,
                    confirmed=moon_tx.confirmed,
                    sent=moon_tx.sent,
                    spend_bundle=None,
                    additions=moon_tx.additions,
                    removals=moon_tx.removals,
                    wallet_id=moon_tx.wallet_id,
                    sent_to=moon_tx.sent_to,
                    trade_id=moon_tx.trade_id,
                    type=moon_tx.type,
                    name=moon_tx.name,
                    memos=[],
                )
