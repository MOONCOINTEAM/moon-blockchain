...
from moon.cmds.init_funcs import check_keys, moon_init, moon_full_version_str
from moon.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from moon.daemon.keychain_server import KeychainServer, keychain_commands
from moon.daemon.windows_signal import kill
from moon.plotters.plotters import get_available_plotters
from moon.plotting.util import add_plot_directory
from moon.server.server import ssl_context_for_root, ssl_context_for_server
from moon.ssl.create_ssl import get_mozilla_ca_crt
from moon.util.moon_logging import initialize_logging
from moon.util.config import load_config
from moon.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
from moon.util.json_util import dict_to_json_str
from moon.util.keychain import (
    Keychain,
    passphrase_requirements,
    supports_os_passphrase_storage,
)
from moon.util.lock import Lockfile, LockfileError
from moon.util.service_groups import validate_service
from moon.util.setproctitle import setproctitle
from moon.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Moon.")
    quit()


log = logging.getLogger(__name__)

service_plotter = "moon_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "moon": "moon",
        "moon_data_layer": "start_data_layer",
        "moon_data_layer_http": "start_data_layer_http",
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Moon will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _moonpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "moonpos")
        command_args: List[str] = ["moon", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "moonpos":
            command_args.extend(self._moonpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "moonpos")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered MOON_ROOT
    os.environ["MOON_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with MOON_ROOT: {os.environ['MOON_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in moon_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    moon_init(root_path, should_check_keys=(not wait_for_unlock))
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"moon-blockchain version: {moon_full_version_str()}")
