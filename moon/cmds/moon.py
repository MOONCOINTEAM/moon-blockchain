...
from moon.cmds.configure import configure_cmd
from moon.cmds.farm import farm_cmd
from moon.cmds.data import data_cmd
from moon.cmds.init import init_cmd
from moon.cmds.keys import keys_cmd
from moon.cmds.netspace import netspace_cmd
from moon.cmds.passphrase import passphrase_cmd
from moon.cmds.plots import plots_cmd
from moon.cmds.rpc import rpc_cmd
from moon.cmds.show import show_cmd
from moon.cmds.start import start_cmd
from moon.cmds.stop import stop_cmd
from moon.cmds.wallet import wallet_cmd
from moon.cmds.plotnft import plotnft_cmd
from moon.cmds.plotters import plotters_cmd
from moon.cmds.db import db_cmd
from moon.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from moon.util.errors import KeychainCurrentPassphraseIsInvalid
from moon.util.keychain import (
    Keychain,
    set_keys_root_path,
)
from moon.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage moon blockchain infrastructure ({__version__})\n",
    epilog="Try 'moon start node', 'moon netspace -d 192', or 'moon show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from moon.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", short_help="Show moon version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs moon daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by moon start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from moon.daemon.server import async_run_daemon
