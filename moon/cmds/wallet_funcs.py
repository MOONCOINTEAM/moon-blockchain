...
from moon.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from moon.cmds.show import print_connections
from moon.cmds.units import units
from moon.rpc.wallet_rpc_client import WalletRpcClient
from moon.server.start_wallet import SERVICE_NAME
from moon.types.blockchain_format.sized_bytes import bytes32
from moon.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from moon.util.config import load_config, selected_network_address_prefix
...
from moon.wallet.trading.trade_status import TradeStatus
from moon.wallet.transaction_record import TransactionRecord
from moon.wallet.util.address_type import AddressType, ensure_valid_address
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        moon_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {moon_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["moon"]))
    final_amount: uint64
    final_min_coin_amount: uint64
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["moon"]))
        final_min_coin_amount = uint64(int(min_coin_amount * units["moon"]))
    ...
    print(f"To get status, use command: moon wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["moon"])
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use moon wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_offer_summary(cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False):
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["moon"]
        wid: str = "1" if asset_id == "moc" else ""
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['moon']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        if nft_royalty_asset_id == "moc":
            nft_royalty_currency = "MOC"
        else:
            result = await cat_name_resolver(bytes32.fromhex(nft_royalty_asset_id))
            if result is not None:
                nft_royalty_currency = result[1]

        nft_royalty_divisor = units["moon"] if nft_royalty_asset_id == "moc" else units["cat"]
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['moon']}")
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use moon wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use moon wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG, WalletType.RATE_LIMITED]:
        return address_prefix, units["moon"]
    return "", units["mojo"]
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.MOC}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.MOC}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.MOC}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["moon"])
