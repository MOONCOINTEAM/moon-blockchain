...
from moon.util.misc import format_bytes
from moon.util.misc import format_minutes
from moon.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total moon farmed: {amounts['farmed_amount'] / units['moon']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['moon']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['moon']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'moon start wallet' and 'moon wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'moon wallet show'")
    else:
        print("Note: log into your key using 'moon wallet show' to see rewards for each key")
