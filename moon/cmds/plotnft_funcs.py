...
from moon.cmds.wallet_funcs import print_balance, wallet_coin_unit
from moon.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
from moon.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
...
from moon.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
from moon.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["moon"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  moon start -r farmer")
        return
    print("Aborting.")
