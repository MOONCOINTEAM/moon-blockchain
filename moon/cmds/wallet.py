...
from moon.cmds.plotnft import validate_fee
...
def get_transaction_cmd(wallet_rpc_port: Optional[int], fingerprint: int, id: int, tx_id: str, verbose: int) -> None:
    ...
    from moon.cmds.cmds_util import execute_with_wallet
...
@wallet_cmd.command("send", short_help="Send moon to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much moon to send, in MOC", type=str, required=True)
...
@click.option("-t", "--address", help="Address to send the MOC", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min_coin_amount",
    help="Ignore coins worth less then this much MOC or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@wallet_cmd.command("make_offer", short_help="Create an offer of MOC/CATs for MOC/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in MOC.",
    type=str,
    default="0",
    show_default=True,
