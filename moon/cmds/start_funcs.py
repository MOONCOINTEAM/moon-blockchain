...
from moon.cmds.keys_funcs import migrate_keys
from moon.cmds.passphrase_funcs import get_current_passphrase
from moon.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from moon.util.errors import KeychainMaxUnlockAttempts
from moon.util.keychain import Keychain
from moon.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    moon = sys.argv[0]
    process = subprocess.Popen(f"{moon} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(
    ...
    if daemon is None:
        print("Failed to create the moon daemon")
        return None
