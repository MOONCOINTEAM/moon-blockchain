...
from moon.util.config import (
    create_default_moon_config,
    initial_config_file,
    load_config,
...
from moon.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'moon keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "moc_target_address" not in config["farmer"]
        stop_searching_for_pool = "moc_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("moc_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("moc_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("moc_target_address") or all_targets[-2] == config["pool"].get(
                    "moc_target_address"
                ):
        ...
        if "moc_target_address" not in config["farmer"]:
            print(
                f"Setting the moc destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["moc_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["moc_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['moc_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "moc_target_address" not in config["pool"]:
            print(f"Setting the moc destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["moc_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["moc_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['moc_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the MOC destination addresses, edit the `moc_target_address` entries in"
...
def create_all_ssl(
    ...
    *,
    ...
    moon_ca_crt, moon_ca_key = get_moon_ca_crt_key()
    moon_ca_crt_path = ca_dir / "moon_ca.crt"
    moon_ca_key_path = ca_dir / "moon_ca.key"
    write_ssl_cert_and_key(moon_ca_crt_path, moon_ca_crt, moon_ca_key_path, moon_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return moon_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def moon_version_number() -> Tuple[str, str, str, str]:
...
def moon_minor_release_number():
    res = int(moon_version_number()[2])
    print(f"Install release number: {res}")
    return res


def moon_full_version_str() -> str:
    major, minor, patch, dev = moon_version_number()
...
def moon_init(
    ...
    *,
    ...
    """
    moon_root = os.environ.get("MOON_ROOT", None)
    if moon_root is not None:
        print(f"MOON_ROOT is set to {moon_root}")

    print(f"Moon directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if MOON_ROOT is set, or if user has run moon init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'moon keys show --show-mnemonic-seed'")
