...
electron-packager . Moon --asar.unpack="**/daemon/**" --platform=darwin \
--icon=src/assets/img/Moon.icns --overwrite --app-bundle-id=net.moon.blockchain \
--appVersion=$MOON_INSTALLER_VERSION \
...
ls -l "${MAC_PACKAGE_NAME}/Moon.app/Contents/Resources/app.asar"
...
if [ "$NOTARIZE" == true ]; then
  electron-osx-sign "${MAC_PACKAGE_NAME}/Moon.app" --platform=darwin \
  --hardened-runtime=true --provisioning-profile=moonblockchain.provisionprofile \
...
DMG_NAME="Moon-${MOON_INSTALLER_VERSION}${MAC_FILE_SUFFIX}.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
NODE_PATH=./npm_macos/node_modules node build_dmg.js "dist/$MAC_PACKAGE_NAME/Moon.app" "${MOON_INSTALLER_VERSION}${MAC_FILE_SUFFIX}"
LAST_EXIT_CODE=$?
if [ "$LAST_EXIT_CODE" -ne 0 ]; then
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id net.moon.blockchain \
...
# xcrun altool --notarize-app -f Moon-0.1.X.dmg --primary-bundle-id net.moon.blockchain -u username -p password
...
# xcrun stapler staple Moon-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Moon-0.1.X.dmg
