...
# Builds CLI only rpm
CLI_RPM_BASE="moon-blockchain-cli-$MOON_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/moon"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/moon/"
ln -s ../../opt/moon/moon "dist/$CLI_RPM_BASE/usr/bin/moon"
...
fpm -s dir -t rpm \
  ...
  --name moon-blockchain-cli \
  --license Apache-2.0 \
  --version "$MOON_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Moon is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create moon-blockchain-$MOON_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/MOONCOINTEAM/moon-blockchain/issues/3846
